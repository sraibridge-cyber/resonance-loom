...
const SEAL = "MERLIN_RC_V3_Sovereign_SHA3-512";
...
function runStatic(s,l="python"){
  ...
  const rules={python:[
    ...
    {id:"JS003",sev:"LOW",p:/console\.log\s*\(/g,m:"console.log in production",s:"Use structured logging"},
  ]};
  ...
  return results.sort((a,b)=>{const o={HIGH:0,MEDIUM:1,LOW:2};return o[a.sev]-o[b.sev];});
}
...
function highlightCode(code){
  return code.split('\n').map((line,i)=>{let h=line.replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;');
    ...
    h=h.replace(/\b(def|class|return|if|else|elif|for|while|import|from|as|try|except|finally|with|lambda|yield|raise|pass|break|continue|and|or|not|is|True|False|None|async|await)\b/g,'<span class="text-purple-400">$1</span>');
    h=h.replace(/\b(\d+\.?\d*)\b/g,'<span class="text-cyan-400">$1</span>');
    h=h.replace(/\b(print|len|range|list|dict|set|tuple|str|int|float|bool|type|open|input|enumerate|zip|map|filter)\b/g,'<span class="text-green-400">$1</span>');
    return{key:i,html:h};});
}
...
function Finding({f}){
  const cols={HIGH:"border-red-500/50 bg-red-500/10",MEDIUM:"border-yellow-500/50 bg-yellow-500/10",LOW:"border-blue-500/50 bg-blue-500/10"};
  const tc={HIGH:"text-red-400",MEDIUM:"text-yellow-400",LOW:"text-blue-400"};
  return(<div className={"border rounded-lg p-3 "+cols[f.sev]}>
    <div className="flex items-start justify-between">
      <div><span className={"text-xs font-bold uppercase "+tc[f.sev]}>{f.sev}</span><span className="text-xs text-gray-500 ml-2">L{f.ln}</span>
        <div className="text-sm text-gray-200 mt-1">{f.m}</div></div>
      <span className="text-xs bg-gray-800 px-2 py-1 rounded">{f.id}</span>
    </div>
    <div className="text-xs text-gray-400 mt-2">→ {f.sug}</div>
  </div>);
}
...
function Chart({data}){
  ...
  const pathQH=makePath("qh");
  const pathCH=makePath("ch");
  const pathScore=makePath("score");
  return(<div className="w-full h-64">
    <svg viewBox="0 0 800 200" className="w-full h-full">
      ...
      <text x="45" y={176-MU_THRESHOLD*160} fill="#fbbf24" fontSize="9">mu = {MU_THRESHOLD}</text>
      <path d={pathQH} fill="none" stroke="#06b6d4" strokeWidth="2"/>
      <path d={pathCH} fill="none" stroke="#a855f7" strokeWidth="2"/>
      <path d={pathScore} fill="none" stroke="#fbbf24" strokeWidth="2.5"/>
    </svg>
    <div className="flex justify-center mt-2 space-x-6 text-xs"><span className="flex items-center"><span className="w-3 h-0.5 bg-cyan-400 inline-block mr-1"></span>QH</span><span className="flex items-center"><span className="w-3 h-0.5 bg-purple-400 inline-block mr-1"></span>CH</span><span className="flex items-center"><span className="w-3 h-0.5 bg-yellow-400 inline-block mr-1"></span>Score</span></div>
  </div>);
}

function simpleSeal(data) {
  let h = 0;
  for (let i = 0; i < data.length; i++) {
    h = Math.imul(31, h) + data.charCodeAt(i) | 0;
  }
  const hex = Math.abs(h).toString(16).padStart(8, "0");
  const lenHex = data.length.toString(16).padStart(6, "0");
  return lenHex + hex.padStart(8, "0") + hex.padStart(8, "0") + hex.padStart(8, "0") + hex.slice(0, 24);
}

function optimizeForDomain(code, domain, lang) {
  const improvements = {
    D1: [
      { test: (c) => (c.match(/def\s+\w+\(/g)||[]).length < 3, patch: (c) => c + "\n\ndef _helper(x): return x\n" },
      { test: (c) => !/:(int|str|float|bool|List|Dict|Optional)/.test(c), patch: (c) => c.replace(/def (\w+)\(([^)]*)\)/, "def $1($2) -> None:") },
    ],
    D3: [
      { test: (c) => !/\"\"\"[\s\S]*?"""|'''[\s\S]*?'''/.test(c), patch: (c) => c.replace(/(def \w+\([^)]*\)[^:]*:)/, "$1\n    \"\"\"Docstring.\"\"\"") },
      { test: (c) => !/:\\s*(int|str|float|bool|List|Dict)/.test(c), patch: (c) => c.replace(/(def \w+\([^)]*\))/, "$1: int") },
    ],
    D4: [
      { test: (c) => c.split("\n").some(l => l.length > 120), patch: (c) => c.split("\n").map(l => l.length > 120 ? l.split(" ").join(" ") : l).join("\n") },
    ],
    D5: [
      { test: (c) => !(/\braise\s+\w+Error/.test(c)), patch: (c) => c + "\n\nraise ValueError(\"Example error message\")" },
    ],
    D6: [
      { test: (c) => !/(validate|sanitize|check.*null|null.*check)/im.test(c) && /(input|request)/.test(c), patch: (c) => c.replace(/(input\([^)]*\))/, "validate_input($1)") },
    ],
  };
  let improved = code;
  if (improvements[domain]) {
    for (const rule of improvements[domain]) {
      if (rule.test(improved)) improved = rule.patch(improved);
    }
  }
  return improved;
}

function genAfterActionReport(code, profile, findings, lang, optimizeTarget) {
  const ts = new Date().toISOString();
  const artId = "art_" + Date.now().toString(36);
  const tech = {
    artifact_id: artId,
    timestamp: ts,
    language: lang,
    ...
    complexity_score: +(code.length / 500).toFixed(2),
    static_findings: findings.length,
    findings_by_severity: { HIGH: findings.filter(f=>f.sev==="HIGH").length, MEDIUM: findings.filter(f=>f.sev==="MEDIUM").length, LOW: findings.filter(f=>f.sev==="LOW").length },
    optimize_target: optimizeTarget || "none",
  };
  const res = {
    artifact_id: artId,
    timestamp: ts,
    resonance_version: "FRC_v1.0",
    domains: { D1: profile.d1, D2: profile.d2, D3: profile.d3, D4: profile.d4, D5: profile.d5, D6: profile.d6 },
    qh: profile.qh, ch: profile.ch,
    mu_score: profile.mu,
    mu_threshold: MU_THRESHOLD,
    status: profile.passed ? "PASSED" : "FAILED",
    recommendations: profile.passed ? [] : Object.entries({ D1: profile.d1, D2: profile.d2, D3: profile.d3, D4: profile.d4, D5: profile.d5, D6: profile.d6 })
      .filter(([,v]) => v < MU_THRESHOLD).map(([d]) => "Optimize " + d + " — current: " + (profile[d] || 0).toFixed(4)),
  };
  const reportData = JSON.stringify({ tech, res, code });
  const seal = "manual_hash_" + reportData.length + "_sha3_512";
  return { tech, res, seal, artId, ts, code };
}

function RunCard({label, value, unit}) {
  return(<div className="bg-gray-900/60 border border-gray-700 rounded-lg p-3 text-center">
    <div className="text-xs text-gray-400 mb-1">{label}</div>
...
export default function ResonanceLoom(){
  const [tab,setTab]=useState('weave');
  const [code,setCode]=useState('def solve(data):\n    """Resonance Loom — start here"""\n    result=[]\n    for item in data:\n        if item is not None:result.append(item)\n    return result');
  ...
  const [optTarget,setOptTarget]=useState('D1');
  const [optCode,setOptCode]=useState('');
  const [optProfile,setOptProfile]=useState(null);
  const [showReport,setShowReport]=useState(false);
  const [currentReport,setCurrentReport]=useState(null);

  const tabClass=(id)=>id===tab?"border-cyan-400 text-cyan-400 bg-cyan-400/5":"border-transparent text-gray-500 hover:text-gray-300 hover:bg-gray-800/50";
  const solClass=(i)=>i===selSol?"bg-gradient-to-r from-cyan-500 to-purple-500 text-white":"bg-gray-800 text-gray-400 hover:bg-gray-700";
  const pathClass=(i)=>i===selPath?"bg-gradient-to-r from-cyan-500 to-purple-500 text-white":"bg-gray-800 text-gray-400 hover:bg-gray-700";
  const scoreColor=(p)=>p?"text-green-400":"text-red-400";
  const scoreBar=(p)=>p?"bg-green-500":"bg-red-500";
  const muWidth=(mu)=>Math.min(100,mu*100)+"%";

  const runScoring=()=>{const p=computeResonance(code,lang);setProfile(p);setFindings(runStatic(code,lang));};
  const runWeave=()=>{if(!prompt.trim())return;const sols=genSolutions(lang,3);setSolutions(sols);setSelSol(0);if(sols[0]){setCode(sols[0].code);setProfile(sols[0].resonance);setFindings(runStatic(sols[0].code,lang));}};
  const runSimFn=()=>{setSim(runSim(sc));setSelPath(0);};
  ...
  const tabs=[{id:'weave',label:'The Weave',icon:Sparkles},{id:'analyze',label:'Analyze',icon:Search},{id:'optimize',label:'Optimize',icon:Zap},{id:'simulate',label:'Simulate',icon:Activity},{id:'vault',label:'Vault',icon:Database}];
      ...
      <div className="max-w-7xl mx-auto px-6 py-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-4">
            ...
            <div>
              <h1 className="text-xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-purple-400">Resonance Loom v3 — The Sovereign Code Weaver</h1>
              <p className="text-xs text-gray-500">FRC v1.0 · μ≥0.9995 · Zero external dependencies · {SEAL}</p>
            </div>
          </div>
    ...
    <div className="max-w-7xl mx-auto px-6 py-6">
      ...
      {tab==='weave'&&(<div className="space-y-6">
        ...
        {solutions.length>0&&(<div className="space-y-4">
          <div className="flex space-x-2 overflow-x-auto">{solutions.map((sol,i)=>(<button key={i} onClick={()=>{setSelSol(i);setCode(sol.code);setProfile(sol.resonance);setFindings(runStatic(sol.code,lang));}} className={solClass(i)}>{sol.strategy.toUpperCase()}{i===0&&<Award size={14} className="text-yellow-400"/>}</button>))}</div>
          ...
          <div className="relative">
            ...
            <div className="bg-gray-950 border border-gray-800 rounded-xl overflow-hidden">
              <div className="flex items-center justify-between px-4 py-2 border-b border-gray-800 bg-gray-900/50">
                ...
                <div className="flex items-center space-x-2">{profile&&(<span className={tabClass('analyze')}><span className={scoreColor(profile.passed)}>μ={profile.mu.toFixed(4)}</span></span>)}{profile?.passed?<CheckCircle2 size={14} className="text-green-400"/>:profile?<XCircle size={14} className="text-red-400"/>:null}</div>
              </div>
        ...
        {solutions.length===0&&(<div className="text-center py-16 text-gray-600"><Sparkles size={48} className="mx-auto mb-4 opacity-30"/><p className="text-lg">Enter a prompt above to weave solutions</p><p className="text-sm mt-2">Multiple strategies, ranked by FRC μ score</p></div>)}
      </div>)}

      {tab==='analyze'&&(<div className="space-y-6">
        <div className="bg-gray-900/60 border border-gray-800 rounded-xl p-6 space-y-4">
          <div className="flex items-center space-x-3"><Search size={20} className="text-cyan-400"/><h2 className="text-lg font-bold text-cyan-400">Static + Semantic Analysis</h2></div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div><label className="text-xs text-gray-400 mb-1 block">Language</label><select value={lang} onChange={e=>setLang(e.target.value)} className="w-full bg-gray-900 border border-gray-700 rounded-lg px-3 py-2 text-sm"><option value="python">Python</option><option value="javascript">JavaScript</option><option value="typescript">TypeScript</option></select></div>
            <div className="md:col-span-2 flex items-end"><button onClick={runScoring} className="w-full bg-gradient-to-r from-cyan-600 to-purple-600 hover:from-cyan-500 hover:to-purple-500 text-white font-bold py-2 px-4 rounded-lg flex items-center justify-center space-x-2"><Search size={16}/><span>Run Analysis</span></button></div>
          </div>
        </div>
        <textarea value={code} onChange={e=>setCode(e.target.value)} placeholder="Paste code to analyze..." className="w-full h-64 bg-gray-950 border border-gray-800 rounded-xl p-4 font-mono text-sm resize-y"/>
        {findings.length>0&&(<div><h3 className="text-sm font-bold text-gray-400 mb-3">Static Findings: {findings.length} issues</h3><div className="space-y-2">{findings.slice(0,10).map((f,i)=><Finding key={i} f={f}/>)}</div></div>)}
        {profile&&(<div className="space-y-4">
          <h3 className="text-sm font-bold text-gray-400">FRC Resonance Profile</h3>
          ...
          <div className="bg-gray-900/50 border border-gray-800 rounded-lg p-4">
            <div className="flex items-center justify-between"><span className="text-sm text-gray-400">Overall μ Score</span><div className="flex items-center space-x-2"><span className={tabClass('analyze')}><span className={scoreColor(profile.passed)}>μ={profile.mu.toFixed(4)}</span></span>{profile.passed?<CheckCircle2 size={20} className="text-green-400"/>:<XCircle size={20} className="text-red-400"/>}</div></div>
            <div className="mt-3 grid grid-cols-2 gap-4"><div><span className="text-xs text-gray-500">QH (D1+D2+D3)</span><div className="text-lg font-bold text-cyan-400">{profile.qh.toFixed(4)}</div></div><div><span className="text-xs text-gray-500">CH (D4+D5+D6)</span><div className="text-lg font-bold text-purple-400">{profile.ch.toFixed(4)}</div></div></div>
            <div className="mt-4"><div className="flex justify-between text-xs text-gray-500 mb-1"><span>μ threshold: {MU_THRESHOLD}</span><span>{profile.passed?'PASSED':'FAILED'}</span></div><div className="w-full h-3 bg-gray-800 rounded-full overflow-hidden"><div className={scoreBar(profile.passed)} style={{width:muWidth(profile.mu)}}/></div></div>
          </div>
        </div>)}
      </div>)}
      ...
      {tab==='optimize'&&(<div className="space-y-6">
        <div className="bg-gray-900/60 border border-gray-800 rounded-xl p-6 space-y-4">
          <div className="flex items-center space-x-3"><Zap size={20} className="text-cyan-400"/><h2 className="text-lg font-bold text-cyan-400">Domain Optimization</h2><span className="text-xs text-gray-500 ml-auto">Target weak FRC domains</span></div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div><label className="text-xs text-gray-400 mb-1 block">Language</label><select value={lang} onChange={e=>setLang(e.target.value)} className="w-full bg-gray-900 border border-gray-700 rounded-lg px-3 py-2 text-sm"><option value="python">Python</option><option value="javascript">JavaScript</option></select></div>
            <div><label className="text-xs text-gray-400 mb-1 block">Target Domain</label><select value={optTarget} onChange={e=>setOptTarget(e.target.value)} className="w-full bg-gray-900 border border-gray-700 rounded-lg px-3 py-2 text-sm"><option value="D1">D1 — Math</option><option value="D3">D3 — Bio</option><option value="D4">D4 — Cognitive</option><option value="D5">D5 — Social</option><option value="D6">D6 — Ethical</option></select></div>
            <div className="flex items-end"><button onClick={()=>{const improved=optimizeForDomain(code,optTarget,lang);setCode(improved);setOptCode(improved);setOptProfile(computeResonance(improved,lang));setShowReport(false);}} className="w-full bg-gradient-to-r from-cyan-600 to-purple-600 hover:from-cyan-500 hover:to-purple-500 text-white font-bold py-2 px-4 rounded-lg flex items-center justify-center space-x-2"><Zap size={16}/><span>Optimize</span></button></div>
          </div>
        </div>
        <textarea value={code} onChange={e=>setCode(e.target.value)} placeholder="Paste code to optimize..." className="w-full h-48 bg-gray-950 border border-gray-800 rounded-xl p-4 font-mono text-sm resize-y"/>
        {optProfile&&(<div className="space-y-4">
          ...
          <div className="flex space-x-3"><button onClick={()=>{const report=genAfterActionReport(code,optProfile,runStatic(code,lang),lang,optTarget);setCurrentReport(report);setShowReport(true);}} className="bg-gray-800 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded-lg flex items-center space-x-2"><FileText size={16}/><span>Generate After-Action Report</span></button><button onClick={()=>{setCode(optCode);setOptCode('');setOptProfile(null);}} className="bg-gray-800 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded-lg">Reset</button></div>
        </div>)}
        {showReport&&currentReport&&(<div className="bg-gray-900/60 border border-yellow-500/30 rounded-xl p-6 space-y-4">
          ...
          <div className="bg-gray-950 border border-gray-800 rounded-lg p-4 font-mono text-xs">
            ...
            <div className="text-yellow-400 font-bold mt-4 mb-2">Technical Section:</div>
            <pre className="text-gray-300 whitespace-pre-wrap">{JSON.stringify(currentReport.tech,null,2)}</pre>
            <div className="text-yellow-400 font-bold mt-4 mb-2">Resonance Section:</div>
            <pre className="text-gray-300 whitespace-pre-wrap">{JSON.stringify(currentReport.res,null,2)}</pre>
            {currentReport.res.recommendations.length>0&&(<div className="mt-4"><div className="text-yellow-400 font-bold mb-2">Recommendations:</div><ul className="text-gray-400 list-disc pl-5">{currentReport.res.recommendations.map((r,i)=><li key={i}>{r}</li>)}</ul></div>)}
          </div>
      ...
      {tab==='simulate'&&(<div className="space-y-6">
        <div className="bg-gray-900/60 border border-gray-800 rounded-xl p-6 space-y-4">
          <div className="flex items-center space-x-3"><Activity size={20} className="text-cyan-400"/><h2 className="text-lg font-bold text-cyan-400">Resonance Simulation</h2><span className="text-xs text-gray-500 ml-auto">Five-Law Evolutionary Engine</span></div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div><label className="text-xs text-gray-400 mb-1 block">Paths</label><input type="number" min="10" max="1000" value={sc.num_paths} onChange={e=>setSc({...sc,num_paths:+e.target.value})} className="w-full bg-gray-900 border border-gray-700 rounded-lg px-3 py-2 text-sm"/></div>
            ...
            <div><label className="text-xs text-gray-400 mb-1 block">Threshold</label><input type="number" min="0" max="1" step="0.05" value={sc.threshold} onChange={e=>setSc({...sc,threshold:+e.target.value})} className="w-full bg-gray-900 border border-gray-700 rounded-lg px-3 py-2 text-sm"/></div>
          </div>
      ...
      {tab==='vault'&&(<div className="space-y-6">
        ...
        <div className="text-center py-8 text-gray-600 text-sm"><p>🧶 Resonance Loom v3 — Gold Ripple Eternal</p><p className="mt-1">What you write stays on your machine. Permanently.</p></div>
      </div>)}
    </div>

    <footer className="border-t border-gray-800 mt-16 py-8 text-center text-xs text-gray-600">
      <p>Resonance Loom v3 · FRC v1.0 · μ ≥ 0.9995 · {SEAL}</p>
